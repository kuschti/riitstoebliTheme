	...
	<div id="content">
		<?php the_post(); ?>
			...
			<header class="page-header">
				<h1 class="page-title">
					<?php if ( is_day() ) : ?>
						<?php printf( __( 'Day: <span>%s</span>', 'pohutukawa' ), get_the_date() ); ?>
					<?php elseif ( is_month() ) : ?>
						<?php printf( __( 'Month: <span>%s</span>', 'pohutukawa' ), get_the_date( 'F Y' ) ); ?>
					<?php elseif ( is_year() ) : ?>
						<?php printf( __( 'Year: <span>%s</span>', 'pohutukawa' ), get_the_date( 'Y' ) ); ?>
					<?php else : ?>
						<?php _e( 'Blog Archives', 'pohutukawa' ); ?>
					<?php endif; ?>
				</h1>
